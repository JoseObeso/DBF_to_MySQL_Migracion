...
text to lqry_ver_clientes noshow
  SELECT *  FROM bdcontratos.clientes_sigesco_bdcontrato where sigesco_cod_externo is null ORDER BY sigesco_razon_social
ENDTEXT
lejecutabusca = sqlexec(gconecta,lqry_ver_clientes, "tver_sigesco")
SELECT tver_sigesco
GO top
SCAN
    lc_nomper = '%' + ALLTRIM(tver_sigesco.sigesco_razon_social) + '%'
    lc_nomper_unico = ALLTRIM(tver_sigesco.sigesco_razon_social)
    TEXT TO lqry_ubicar noshow
      SELECT * FROM bdcontratos.mpersonas where cnomper like  ?lc_nomper   limit 1;
    ENDTEXT
    ...
    TEXT TO lqry_update_sigesco noshow
      ...
      bdcontratos_ruc = ?lc_ruc, bdcontratos_tipo = ?lc_tipo, bdcontratos_distrito = ?lc_distri, bdcontratos_direccion = ?lc_direccion, marcar_null = ?lc_null
       where sigesco_razon_social = ?lc_nomper_unico
    ENDTEXT
